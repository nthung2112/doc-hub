 ...
 */
function panDiagram(
  panzoom: Panzoom.PanzoomObject | null,
...
function DiagramModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal Content */}
      <div className="relative bg-white border border-gray-300 rounded-lg shadow-xl w-[calc(100%-20px)] max-h-[90%] overflow-auto animate-in fade-in-0 zoom-in-95 duration-300">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-2 bg-gray-50/50 border-b">
          <h3 className="text-lg font-semibold !m-0">Diagram Viewer</h3>
          <button
            onClick={onClose}
            type="button"
...
export function PreviewPaneContainer({ code, chart }: PreviewPaneProps) {
  ...
  return (
    <>
      <PreviewPane chart={chart} code={code} onOpenModal={handleOpenModal} />
...
export function PreviewPane({
  ...
  const panzoomRef = useRef<Panzoom.PanzoomObject | null>(null);

  // Handle wheel events for zooming
  const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    if (!event.shiftKey || !panzoomRef.current) return;
  ...
  useEffect(() => {
    ...
    // Initialize panzoom
    const panzoom = Panzoom.default(diagram);
  ...
  return (
    <div className="relative" ref={containerRef} onWheel={handleWheel}>
      <div ref={diagramRef} dangerouslySetInnerHTML={{ __html: code }} />
      {/* View Control Panel */}
      <div
        role="toolbar"
      ...
      {!isInModal && (
        <div
          role="toolbar"
          className="absolute top-4 right-4 grid gap-1 grid-cols-2"
        >
          <ControlButton
